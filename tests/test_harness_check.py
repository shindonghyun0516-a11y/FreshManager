...
from scripts import harness_check as harness
...
def valid_rows() -> list[dict[str, str]]:
    ...
    number = 1
    for category, count in harness.EXPECTED_CATEGORIES.items():
...
def valid_sample() -> dict[str, object]:
    current = {field: "1" for field in harness.CURRENT_POPULATION_FIELDS}
    current.update(
        {
            "AREA_NM": "여의도",
            "AREA_CD": "POI072",
...
def write_csv(path: Path, rows: list[dict[str, str]], headers: list[str] | None = None) -> None:
    ...
    fieldnames = headers or list(harness.EXPECTED_HEADERS)
...
class TemporaryProject:
    def __init__(self) -> None:
        ...
        self.csv_path = self.root / harness.CSV_RELATIVE_PATH
        self.json_path = self.root / harness.JSON_RELATIVE_PATH
        write_csv(self.csv_path, valid_rows())
        write_json(self.json_path, valid_sample())

    def context(self) -> harness.HarnessContext:
...
- EG-3 Python 하네스 구현 완료

## 9. 단계적 구현 순서

{harness.EG3_STATUS_ROW}

## 18. 현재 실행방법

```bash
{harness.STANDARD_HARNESS_COMMAND}
```
...
class H004ReadmeStatusTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.harness_path = self.project.root / "scripts/harness_check.py"
        self.harness_path.parent.mkdir(parents=True, exist_ok=True)
        self.harness_path.write_text("# test harness\n", encoding="utf-8")
        self.readme_path.write_text(valid_h004_readme(), encoding="utf-8")

    def tearDown(self) -> None:
        self.project.close()

    def check(self) -> harness.CheckResult:
        return harness.check_h004(self.project.context())

    def test_missing_harness_file_fails(self) -> None:
        self.harness_path.unlink()
        self.assertEqual(self.check().status, harness.Status.FAIL)

    def test_missing_standard_command_fails(self) -> None:
        text = valid_h004_readme().replace(harness.STANDARD_HARNESS_COMMAND, "명령 미기재")
    ...
    def test_eg3_unimplemented_current_status_fails(self) -> None:
        text = valid_h004_readme().replace(
            harness.EG3_STATUS_ROW,
            "| EG-3 | 하네스 구현 및 자동 재검증 | 미구현 |",
        )
        self.readme_path.write_text(text, encoding="utf-8")
        self.assertEqual(self.check().status, harness.Status.FAIL)

    def test_harness_code_missing_current_status_fails(self) -> None:
        text = valid_h004_readme().replace(
            "- EG-3 Python 하네스 구현 완료",
            "- 하네스 코드 없음",
        )
    ...
    def test_matching_harness_and_readme_pass(self) -> None:
        result = self.check()
        self.assertEqual(result.status, harness.Status.PASS, result.evidence)
...
class CsvContractTests(unittest.TestCase):
    ...
    def assert_failed(self, result: harness.CheckResult) -> None:
        self.assertEqual(result.status, harness.Status.FAIL, result.evidence)

    def test_official_csv_passes_all_csv_contracts(self) -> None:
        context = harness.HarnessContext(
            root=ROOT,
            csv_hash_before=file_hash(ROOT / harness.CSV_RELATIVE_PATH),
            json_hash_before=file_hash(ROOT / harness.JSON_RELATIVE_PATH),
        )
        for check_id in range(101, 113):
            result = getattr(harness, f"check_h{check_id}")(context)
            self.assertEqual(result.status, harness.Status.PASS, result.evidence)
    ...
    def test_missing_csv_is_a_validation_failure(self) -> None:
        ...
        result = harness.check_h101(context)
    ...
    def test_missing_required_header_fails(self) -> None:
        ...
        self.assert_failed(harness.check_h103(self.project.context()))

    def test_additional_header_fails(self) -> None:
        write_csv(self.project.csv_path, valid_rows(), harness.EXPECTED_HEADERS + ["EXTRA"])
        self.assert_failed(harness.check_h103(self.project.context()))

    def test_header_order_fails(self) -> None:
        headers = list(harness.EXPECTED_HEADERS)
    ...
    def test_wrong_record_count_fails(self) -> None:
        ...
        self.assert_failed(harness.check_h104(self.project.context()))

    def test_missing_area_code_fails(self) -> None:
        rows = valid_rows()
        rows[0]["AREA_CD"] = ""
        write_csv(self.project.csv_path, rows)
        self.assert_failed(harness.check_h105(self.project.context()))

    def test_duplicate_area_code_fails(self) -> None:
        rows = valid_rows()
        rows[1]["AREA_CD"] = rows[0]["AREA_CD"]
        write_csv(self.project.csv_path, rows)
        self.assert_failed(harness.check_h106(self.project.context()))

    def test_missing_area_name_fails(self) -> None:
        rows = valid_rows()
        rows[0]["AREA_NM"] = ""
        write_csv(self.project.csv_path, rows)
        self.assert_failed(harness.check_h107(self.project.context()))

    def test_wrong_yeouido_code_fails(self) -> None:
        rows = valid_rows()
        rows[71]["AREA_CD"] = "WRONG-CODE"
        write_csv(self.project.csv_path, rows)
        self.assert_failed(harness.check_h108(self.project.context()))

    def test_wrong_category_count_fails(self) -> None:
        rows = valid_rows()
        rows[0]["CATEGORY"] = "공원"
        write_csv(self.project.csv_path, rows)
        self.assert_failed(harness.check_h112(self.project.context()))

    def test_invalid_utf8_fails(self) -> None:
        self.project.csv_path.write_bytes(b"CATEGORY,NO\n\xff,1\n")
        self.assert_failed(harness.check_h102(self.project.context()))

    def test_bom_and_no_bom_are_both_readable(self) -> None:
        write_csv(self.project.csv_path, valid_rows())
        self.assertEqual(harness.check_h102(self.project.context()).status, harness.Status.PASS)
    ...
    def test_area_codes_are_preserved_from_csv_not_no(self) -> None:
        ...
        self.assertEqual(harness.load_area_codes(self.project.csv_path), expected)

    def test_csv_check_does_not_modify_input(self) -> None:
        before = file_hash(self.project.csv_path)
        result = harness.check_h111(self.project.context())
        self.assertEqual(result.status, harness.Status.PASS, result.evidence)
...
class JsonContractTests(unittest.TestCase):
    ...
    def test_missing_json_is_a_validation_failure(self) -> None:
        ...
        self.assert_failed(harness.check_h301(context))

    def test_invalid_json_fails(self) -> None:
        self.use_fixture("json/invalid_json.json")
        self.assert_failed(harness.check_h301(self.project.context()))

    def test_missing_current_population_field_fails(self) -> None:
        self.use_fixture("json/missing_population_field.json")
        self.assert_failed(harness.check_h302(self.project.context()))
    ...
    def test_empty_forecast_array_fails(self) -> None:
        ...
        self.assert_failed(harness.check_h303(self.project.context()))
    ...
    def test_forecast_count_is_informational(self) -> None:
        result = harness.check_h303(self.project.context())
        self.assertEqual(result.status, harness.Status.PASS, result.evidence)
...
class SecurityAndOfflineTests(unittest.TestCase):
    ...
    def test_sensitive_assignment_is_detected_without_value_output(self) -> None:
        ...
        result = harness.check_h203(self.project.context())
        self.assertEqual(result.status, harness.Status.FAIL)
    ...
    def test_real_env_is_never_selected_for_reading(self) -> None:
        ...
        selected = list(harness.iter_security_files(self.project.root))
        self.assertNotIn(env_path, selected)

    def test_full_harness_does_not_connect_to_network(self) -> None:
        with mock.patch("socket.create_connection", side_effect=AssertionError("network forbidden")) as connection:
            results = harness.run_harness(ROOT)
...
class RegistryAndExitCodeTests(unittest.TestCase):
    def test_all_45_ids_are_unique_and_in_spec_order(self) -> None:
        registry = [item.check_id for item in harness.CHECK_DEFINITIONS]
        spec = harness.ids_from_spec(ROOT / harness.SPEC_RELATIVE_PATH)
    ...
    def test_expected_eg3_counts(self) -> None:
        ...
        self.assertEqual(counts[harness.Status.PASS], 28)
        self.assertEqual(counts[harness.Status.FAIL], 0)
        self.assertEqual(counts[harness.Status.WARN], 0)
        self.assertEqual(counts[harness.Status.SKIP], 17)
        self.assertEqual(len(results), 45)

    def test_exit_code_zero_for_success(self) -> None:
        result = harness.make_result("H-404", harness.Status.PASS, "success")
        self.assertEqual(harness.exit_code_for([result]), 0)

    def test_exit_code_one_for_required_failure(self) -> None:
        result = harness.make_result("H-404", harness.Status.FAIL, "failure")
        self.assertEqual(harness.exit_code_for([result]), 1)

    def test_exit_code_two_for_internal_error(self) -> None:
        def raise_internal_error(root: Path) -> list[harness.CheckResult]:
            raise RuntimeError("sensitive details must not be printed")

        stderr = io.StringIO()
        with contextlib.redirect_stderr(stderr):
            exit_code = harness.main(ROOT, runner=raise_internal_error)
    ...
    def test_official_files_are_unchanged_by_full_run(self) -> None:
        csv_path = ROOT / harness.CSV_RELATIVE_PATH
        json_path = ROOT / harness.JSON_RELATIVE_PATH
        before = (file_hash(csv_path), file_hash(json_path))
        harness.run_harness(ROOT)
    ...
    def test_markdown_structure_detects_unclosed_fence_and_heading_jump(self) -> None:
        errors = harness.check_markdown("# Title\n### Jump\n```python\n")
