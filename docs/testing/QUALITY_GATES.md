...
| EG-5 | 진행: Issue #46 작업 Branch에서 대표 3장소 오프라인 구현·검증 중; 실제 세 장소 호출 미승인·미실행 |
| EG-6 | 미구현 |
| EG-7 | 미구현 |
| EG-8 | 미결정 |
...
- 반복수집 전 외장 저장장치 주기적 복사 또는 PM이 승인한 클라우드 폴더 주기적
  백업 중 하나를 별도 Gate로 준비한다. 수집 실행은 로컬에 유지하고 백업만 분리한다.
