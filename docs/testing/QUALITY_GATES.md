...
`HARNESS_SPEC.md`를 유일한 기준으로 사용한다.
...
EG-1과 EG-2의 읽기 전용 사전검증에는 하네스가 필요하지 않다.
...
Python 하네스를 만들거나 실행하지 않고 문서만 검사한다.
...
유일한 공식 장소 기준파일인 CSV를 수집기와 하네스의 입력으로
사용할 수 있는지 확인한다.

### 판정 방법

하네스 구현 전에 파일을 수정하지 않고 읽기 전용으로 확인한다.
관련 하네스 PASS를 요구하지 않는다.
...
- 지정 경로에 공식 CSV가 존재한다.
- `encoding="utf-8-sig"`, `newline=""`와 표준 라이브러리 `csv`로
  UTF-8 또는 UTF-8 BOM 파일을 읽을 수 있다.
- CSV에 필수 컬럼이 모두 존재하고 데이터 행이 정확히 121개다.
...
데이터 사전검증 기준으로 사용한다. 수집기와 하네스 코드가 없는 EG-1에서는
...
하네스 구현 전에 샘플을 수정하지 않고 표준 `json`으로 읽기 전용 확인한다.
관련 하네스 PASS를 요구하지 않는다.
...
- `scripts/harness_check.py`가 구현돼 있다.
- `HARNESS_SPEC.md`의 EG-3 적용 대상 필수검사를 모두 실행한다.
...
- 일반 하네스와 테스트의 네트워크·실제 API 호출이 0회다.
- 적용 대상 필수검사가 모두 PASS이고 종료 코드가 `0`이다.
- 아직 적용되지 않은 기능 검사는 사유를 기록해 SKIP할 수 있지만,
  EG-3 필수검사는 SKIP할 수 없다.
- 결과를 `HARNESS_REPORT_TEMPLATE.md` 형식으로 기록한다.

### 다음 단계

EG-3 오프라인 하네스 통과 후 PM의 EG-4 단계 전환과 여의도 1장소
...
- 샘플 또는 가짜 응답 기반 일반 테스트와 적용 하네스 검사를 통과한다.
- EG-3 오프라인 하네스 통과와 PM 승인 후 실제 `.env`를 생성하고 인증키를 저장한다.
...
- 원본·메타데이터·보안 규칙과 적용 대상 하네스 검사를 통과한다.
