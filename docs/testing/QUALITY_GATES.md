...
| EG-6 | 진행: EG-6A 13개 Area·Spot 패널의 서로 다른 공식 Area 13개 매핑 완료; Diff 검토 전 |
...
완료했으며, EG-6A에서는 실제 API 호출 없이 13개 제안 지역의 Area·Spot·센서 참조를
구현한다. 삼성역은 강남 MICE 관광특구, 광화문역은 광화문광장, 을지로입구역은
...
EG-6A부터 기존 `H-703`을 13지역 참조데이터 무결성 검사로 활성화한다.
...
- 13개 Area가 모두 안전하게 확정되고 PM이 EG-6B 구현과 실제 호출을 각각 승인한다.
- 확정한 동일 13개 코드를 중복·누락 없이 각각 최대 1회 순차 처리한다.
- 장소별 실패 격리, 대상=성공+실패, 실패 목록, 원본·메타데이터 보존을 검증한다.
