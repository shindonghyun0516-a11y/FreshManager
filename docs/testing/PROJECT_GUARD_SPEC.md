...
H-203은 최상위의 정확한 `work log/` 디렉터리를 순회 대상에서 제거한 뒤 파일을
읽으며, 같은 이름이 아닌 다른 로그·문서 디렉터리의 기존 보안검사는 유지한다.
...
EG-5에서는 `H-702`, `H-704`, `H-705`를 활성화하고 `H-707`은 반복수집 전까지
`SKIP`한다. 총 검사 ID는 45개로 유지한다. `H-504`, `H-505`, `H-601`, `H-602`는
Issue #32 결정에 따라 계속 `SKIP`한다.
