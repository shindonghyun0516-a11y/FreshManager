...
이 문서는 총 46개의 고유한 검사 ID를 정의한다. EG-6 MVP 전략 변경에 따라 기존
...
| `H-707` | EG-7 반복주기 승인 준수 | 승인 없는 반복수집과 무백업 운영 방지 | 수집·자동실행·백업 설정, PM 승인 기록 | 반복수집 전 주기 승인을 받고 외장 저장장치 주기적 복사 또는 PM 승인 클라우드 폴더 주기적 백업 중 하나가 별도로 준비되며 수집 자체는 로컬에서 실행 | 승인 전 주기 고정, 백업 Gate 없는 반복수집 또는 수집 실행을 임의로 클라우드로 이전 | EG-6B까지 SKIP 가능, EG-7에서는 불가 | EG-7 이후 |
