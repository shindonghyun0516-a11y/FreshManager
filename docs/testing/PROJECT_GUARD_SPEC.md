...
| `H-706` | EG-6B 13지역 1회 완전성 | 승인 패널의 단일 순차수집 검증 | 승인된 EG-6A Area 패널, 단일 회차 결과·로그 | 승인된 13개 Area를 중복·누락 없이 각각 최대 1회 시도하고 소요시간·성공·실패를 기록 | 대상 누락·중복·임의 코드, 재시도 또는 소요시간 미기록 | EG-6B 전 SKIP만 가능 | EG-6B 이후 |
| `H-707` | EG-7 반복주기 승인 준수 | 승인 없는 반복수집과 백업 운영 없는 반복 실행 방지 | 수집·자동실행·백업 설정, PM 승인 기록 | 반복수집 전 주기·호출량·운영시간 승인을 받고 별도 즉시 Backup Worker의 로컬 복사·무결성 계약과 원격 동기화 확인 절차가 준비되며 수집 자체는 로컬에서 실행 | 승인 전 주기 고정, Backup Worker·원격 확인 절차 없는 반복수집 또는 수집 실행을 임의로 클라우드로 이전 | EG-6B까지 SKIP 가능, EG-7에서는 불가 | EG-7 이후 |
...
`H-701`부터 `H-706`까지와 `H-708`의 일반 테스트는 샘플 또는 가짜 응답만 사용한다.
...
현재 `H-206`, `H-702`, `H-703`, `H-704`, `H-705`, `H-706`, `H-708`을 활성화하고
`H-707`은 반복수집 전까지 `SKIP`한다. 총 검사 ID는 47개다.
