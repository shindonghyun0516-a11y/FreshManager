...
추가하지 않는다. 초기 파일럿의 5개 Area·15개 사용자 선택 Spot 정적 Master도
같은 참조데이터 검사에 포함하며 별도 검사 ID를 만들지 않는다.
...
| `H-703` | EG-6A 참조·파일럿 정적 Spot | 승인되지 않은 Area·Spot·센서 연결과 잘못된 사용자 선택 Spot의 유입 방지 | 공식 CSV, `eg6_area_panel.csv`, `eg6_spot_master.csv`, `eg6_sdot_links.csv`, `pilot_spot_options.csv` | 기존 13개 Area–Anchor–S-DoT 계약을 유지하고 파일럿 5개 Area에 각각 3개·총 15개의 고유 사용자 선택 Spot을 연결함. 주소·서울 범위 좌표·허용 Enum·PM 고정 상태가 유효하고 같은 Area 좌표 중복·기존 Anchor 좌표 복사·추천순위 필드가 없음. 기존 세 CSV에는 URL·민감경로가 없고 파일럿 Master에는 승인된 Issue #132 좌표 댓글 URL만 존재함 | 필수 CSV·헤더 누락, 기존 참조계약 불일치, 파일럿 5×3 구조·ID·Area 참조·주소·좌표·Enum·고정 상태 불일치, 같은 Area 좌표 중복, Anchor 좌표 재사용, 승인되지 않은 URL·민감정보 포함 | EG-6A 전 SKIP만 가능 | EG-6A 이후 |
...
대리좌표 기준 S-DoT 분류, 파일럿 5개 Area·15개 사용자 선택 Spot의 정적 계약을
PASS·FAIL로 판정한다. H-705의 가짜 부분실패는
