...
| `H-003` | 문서 규칙 일치 | 핵심 계약 충돌 방지 | 위 여섯 문서 | 기준파일 역할, EG 순서, PM 승인, 메타데이터, 보안, 오프라인 테스트와 호출주기가 일치하고 현재 상태 절의 EG-4 PASS·EG-5 진행·실제 호출 미실행 표현이 상충하지 않음 | 한 항목 이상 상충하거나 현재 상태표와 본문이 직접 충돌 | 불가 | EG-0, EG-3 이후 |
...
H-206은 EG-3 이후 활성 상태로 `SKIP`할 수 없고 현재 Project Guard는 `TOTAL=46`이다.
...
| `H-707` | 반복주기 승인 준수 | 승인 없는 반복수집과 무백업 운영 방지 | 수집·자동실행·백업 설정, PM 승인 기록 | 반복수집 전 주기 승인을 받고 외장 저장장치 주기적 복사 또는 PM 승인 클라우드 폴더 주기적 백업 중 하나가 별도로 준비되며 수집 자체는 로컬에서 실행 | 승인 전 주기 고정, 백업 Gate 없는 반복수집 또는 수집 실행을 임의로 클라우드로 이전 | EG-7까지 SKIP 가능, EG-8에서는 불가 | EG-8 |

`H-702`, `H-704`, `H-705`는 EG-5부터 활성화한다. H-705의 가짜 부분실패는
...
- EG-5: Issue #46 작업 Branch에서 `POI019`, `POI013`, `POI014` 고정 순차 실행,
  전용 단계 경로, 실패 격리, 재시도 0회와 회차 요약을 오프라인 구현·검증 중이다.
  세 장소 실제 API 호출은 승인되지 않았고 실행하지 않았다.
- EG-6~EG-8: 미진행

EG-5에서는 `H-206`, `H-702`, `H-704`, `H-705`를 활성화하고 `H-707`은
반복수집 전까지 `SKIP`한다. Issue #47에서 H-206을 추가해 총 검사 ID는 46개다.
`H-504`, `H-505`, `H-601`, `H-602`는 Issue #32 결정에 따라 계속 `SKIP`한다.

Issue #47 작업 Branch에서는 저장소 최상위 `work log/`를 완전히 제거하고
`/work log/`를 ignore한다. 신규 작업일지는 저장소 밖에서만 관리하며 기존 추적
항목을 보존하는 Legacy 예외는 없다. Git 이력 재작성은 하지 않고, PR #48은
Issue #47 해결과 전체 재검증 전까지 Merge하지 않는다.
정확 규칙 외의 광범위 ignore는 허용하지 않으며 보호 Git 상태는 원본 stdout·stderr를
출력하지 않고 Boolean·개수로만 보고한다. Issue #47 구현·검증에서는 실제 API 호출과
EG-5 대표 3장소 수집을 수행하지 않는다.
