...
`현재·1시간 후·3시간 후`를 사용하고 내부 `horizon_minutes=60`·`180`과 구분한다.
...
> 초기 웹 파일럿에서는 사용자가 담당 Area를 먼저 선택하고 서울시 공식 현재·1시간 후·
> 3시간 후 Area 정보와 대표 Spot 3개를 확인한 뒤 판촉 후보 위치를 직접 고른다. 현재는
> 이 서비스가 성립할지를
...
해당 Area의 현재·1시간 후·3시간 후 정보와 대표 Spot 3개를 확인해 판촉 후보 위치를 직접
...
> **범위 경계**  원격 근거가 충분하면 특정 Spot과 판매시간을 추천할 수 있다.
> 다만 이 추천은 실제 판매 허용·안전·카트 정차·운영 적합성·판매 성공·매출 증가를
> 보장하지 않는다.
...
초기 파일럿의 Spot 3개는 같은 수준의 사용자 선택지이며 시스템 추천·순위 대상이
아니다. 장기 SPOT 추천에서는 복수 후보의 동적 근거와 반복패턴을 비교해야 한다.
현재 Candidate Anchor를 실제 출구나 판매 위치로 자동 승격하지 않는다.
...
하향하며, Area 근거도 부족하면 추천하지 않는다. 현장검증과 운영 적합성은 추천
Eligibility와 분리해 `UNAVAILABLE`·`NOT_VERIFIED`로 표시한다.
...
- 초기 웹 파일럿의 사용자 담당 Area 선택, 선택 Area의 서울시 공식 현재·1시간 후·
...
→ 선택 Area의 서울시 공식 현재·1시간 후·3시간 후 정보 확인
→ Area당 대표 Spot 3개를 같은 수준으로 표시
→ 사용자가 판촉 후보 Spot 직접 선택
```

별도 시간 선택버튼 없이 1시간 후·3시간 후 정보를 함께 표시한다. 내부 기술계약은
`horizon_minutes=60`·`180`을 유지한다. Area 증감수·증감률, 0분모 처리와 Horizon별
최신성 표시는 `AREA_FIRST_WEB_PILOT_CONTRACT.md`의 단일 계산·표시 계약을 따른다.
D-021 Core·Service는
...
- 판매성과 인과효과 검증
- 실제 판매 허용·안전·카트 정차 가능성·판매 성공·매출 증가 보장
- 승인된 파일럿 범위를 넘어선 13개 Area 전체 구현과 상용 서비스 전체 확대
- 실시간 모델 서빙, 완성형 MLOps
- 프레시매니저 위치 추적, 고객 개인정보 수집 또는 개인별 프로파일링
- 실제 판매량 수집·구매전환율 검증
- 현재 Candidate Anchor의 실제 출구 해석·자동 승격과 이동경로 최적화
...
실제 Spot 좌표와 Area 내부 복수 후보, Spot별 동적·정적 근거, 반복성·순위 안정성,
최신성과 이동 가능시간이 확인되면 원격 데이터 기반 SPOT 추천을 검토할 수 있다.
실제 안전·카트 정차·판매 허용과 운영 적합성은 별도 운영기관 확인 없이는
확정하지 않는다.
...
초기 웹 파일럿은 Responsive Web의 Desktop Web·Mobile Web에서 사용자가 정확히 5개 중
담당 Area를 먼저 선택하고, 선택 Area의 서울시 공식 현재·1시간 후·3시간 후 유동정보를
함께 표시한다(내부 `horizon_minutes=60`·`180`). 태블릿 전용 UI는 만들지 않는다. 각 Area에는 대표 Spot 3개를
...
Context로 구성한다. D-020의 원격 근거 Eligibility를 모두 충족하면 특정 Spot과
판매시간을 `SPOT`으로 추천한다.

- `recommendation_type=SPOT`: 원격 근거로 선택한 특정 이동·판매 대상
- `recommendation_basis=REMOTE_EVIDENCE`: 추천 근거가 원격 데이터임
- `field_verification_status=UNAVAILABLE`: 현재 PoC의 현장검증 불가
- `operational_suitability_status=NOT_VERIFIED`: 판매·안전·정차·운영 적합성 미확인
- 원격 SPOT Eligibility는 같은 Area의 비교 가능한 후보 최소 2개(장기 목표
  3~5개), 공식 명칭·위치, 후보를 구분하는 동적 또는 승인된 대리근거, 동일
  기준시각·시간범위, 반복성 또는 순위 안정성, 최신성·결측, 이동·준비 가능시간,
  신뢰도와 제한사항을 요구한다.
- Spot 근거가 부족하고 Area 근거만 충분하면 `AREA`와 필수 `fallback_reason`을
  사용한다. Area 근거도 부족하면 Recommendation Output을 생성하지 않는다.
...
| 원격 데이터 기반 SPOT 추천 | Area 안 복수 Spot을 비교해 특정 이동·판매 지점과 판매시간을 제시하는 추천; 운영 적합성·판매성과 보장 아님 |
