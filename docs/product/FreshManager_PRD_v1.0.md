...
실제 API 호출, 다음 Engineering Gate, 신규 라이브러리, 기준파일·저장구조·호출주기·자동실행 변경과 main 병합은 PM 명시 승인 후 진행해야 한다.
...
수집주기는 사전에 고정하지 않는다. API 호출한도, 한 회차 소요시간, 실제 갱신주기, 성공률, 운영 컴퓨터 안정성, 분석에 필요한 해상도와 백업 준비를 확인한 뒤 PM이 승인한다.
