...
현재 PoC의 최대 결과는 **데이터 기반 우선 Spot 후보**다. 이는 현장 적합성 확인
Spot이나 실제 판매 가능성이 보장된 Spot과 다르다. 모든 결과에 다음 경계를 적용한다.
...
- 복수 후보·동일 비교기준·반복성이 없으면 `데이터 기반 우선 후보`로 올리지 않는다.
