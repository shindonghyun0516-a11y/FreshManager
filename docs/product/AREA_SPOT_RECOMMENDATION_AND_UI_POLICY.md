...
이 문서는 FreshManager가 Area를 탐색하고 원격자료로 Spot 후보의 우선순위를
검토하는 제품 정책을 정의한다. 정책 문서일 뿐 UI, 추천 계산, 지도 API 또는
사용자 게시를 구현하거나 승인하지 않는다. 추천 결과의 필드·null·fallback 계약은
`docs/product/RECOMMENDATION_OUTPUT_CONTRACT.md`가 계속 소유한다.

## 1. 서비스 목적

FreshManager는 프래시매니저가 어느 Area와 Spot 후보를 추가 검토할지 판단하도록
돕는다. 현장검증이 불가능한 현재 PoC의 최대 출력은 데이터 기반 우선 Spot 후보다.
이는 현장 적합성 확인 Spot이나 실제 판매 가능성이 보장된 Spot이 아니며, 실제
판매 성공이나 매출 효과를 입증하지 않는다.
...
Spot은 Area 안에서 장기적으로 이동·판매 여부를 검토할 구체적인 지점이다.
출구, 오피스 출입구, 버스정류장 인근, 광장 출입구 등이 될 수 있다. Spot은
단순 지도 핀이 아니지만, 현재 원격자료만으로 최종 판매 추천 대상이 되지 않는다.
...
3. Area 안의 Spot 후보와 각 후보의 원격 근거·제한을 비교한다.
4. 조건을 모두 통과하면 데이터 기반 우선 후보로 표시한다.
5. 근거가 부족하면 판매 후보 또는 Area 안내로 하향한다.

## 5. Spot 결과 예시

원격 근거가 충분한 미래 상태의 허용 예시는 다음과 같다.

> 강남역 Area의 유동인구 증가가 예상됩니다. 원격자료 비교에서 A 후보가 우선
> 후보로 나타났지만, 실제 판매 가능성·접근성·안전·판매 제한은 확인되지 않았습니다.

현재 강남역 Anchor를 특정 출구로 해석하지 않는다. 데이터 기반 우선 후보에는
...
| 데이터 기반 우선 후보 | §8의 원격 근거 조건을 모두 충족함 | 운영확인이 필요한 우선 후보로 표시 |
| 추천 제외 | 공식 제한, 오래된 자료, 근거 충돌 또는 품질 결함 | 후보에서 제외 |

현재 집계는 데이터 기반 우선 후보 0개, 판매 후보 13개, 정보 없음 0개, 원격 근거
확인 중 0개, 추천 제외 0개다. 실제 Spot 등록은 0개이며, 운영 적합성은 모두
`NOT_VERIFIED`다.
...
실제 후보 명칭과 공식 또는 검증 가능한 좌표 출처, 같은 Area의 복수 후보, 후보별
동적 근거 또는 승인된 대리근거, 동일 비교기준·시간범위, 반복 시간패턴, 자료
기준시각·최신성, 결측·이상치 관리, 단일 관측에 의존하지 않는 순위와 명시된
제한·불확실성을 모두 요구한다. 하나라도 부족하면 판매 후보 또는 AREA 안내로
하향한다. S-DoT 존재나 후보 수만으로 승격하지 않는다.
...
원격 근거를 모두 충족하면 데이터 기반 우선 후보로 기록한다. 부족하면 판매 후보
또는 Area 안내로 하향한다. 어떤 결과도 판매 가능성이나 운영 적합성을 확정하지 않는다.
...
- Spot 핀은 데이터 기반 우선 후보, 판매 후보, 정보 없음, 원격 근거 확인 중,
  추천 제외를 구분한다.
...
대체 Spot은 그 Spot 자체가 데이터 기반 우선 후보 조건을 통과한 경우에만 제시한다.
...
분석으로 원격 준비도를 평가한다. 향후 실제 운영기관이 별도 현장검증을 수행할
때만 운영 적합성 확인단계를 추가할 수 있다. 역 중심 대리좌표를 출구 또는 판매
위치로 자동 승격하지 않는다.
...
다음 한 단계는 `docs/product/AREA_SPOT_REMOTE_EVIDENCE_READINESS_ASSESSMENT.md`의
세 우선 Area 후보 중 후속 원격 근거 구성을 시작할 Area를 PM이 선택하는 것이다.
UI 구현, 동적 근거 수집, 우선순위 로직과 사용자 게시에는 각각 별도 승인이 필요하다.
