...
레코드 868개가 확인됐다. 공식 CSV는 변경되지 않았으며 재검증 전까지
EG-1 통과로 간주하지 않는다.
...
- 공식 CSV 정정 또는 교체 후 EG-1 읽기 전용 재검증
- API 필드 정의서 작성
- 구현 하네스 작성
- 1개 장소 1회 수집기 작성
- 유형별 대표 3장소 시험수집
...
| EG-1 | 장소 기준데이터 사전검증 | 읽기 전용 검사 완료, 전 필드 공백 레코드 868개로 미통과 |
...
현재는 하네스와 자동수집 코드가 구현되지 않았고 실행 가능한 수집 명령도 없다.
EG-3 오프라인 하네스 통과 전에는 실제 `.env`, 실제 인증키와
실제 서울시 API 호출을 사용하지 않는다.

아직 존재하지 않는 실행 명령을 README에 사실처럼 작성하지 않는다.
