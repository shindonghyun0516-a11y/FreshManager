...
서울시 주요 121장소를 장기 공식 수집 후보군으로 유지한다.
현재 MVP 수집 패널은 EG-6A에서 확정한 13개 Area다.
...
13개 Area 단일·반복 수집과 Feature 분석에서 필요성이 확인된 경우
121개 Area 확대와 장소 유형별·시간대별 비교분석을 후속 검토한다.
...
현재 13개 Area Batch와 후속 승인된 확대 수집은 다음 구조로 실행한다.
...
- 반복수집 주기는 PM이 승인한 5분 벽시계 기준을 고정 적용한다.
- Batch 내부 장소별 호출 사이 대기시간은 반복수집 주기와 구분해 설정값으로 관리한다.
...
동일 13개 Area 반복수집의 장기 기준 주기는 PM 결정으로 다음과 같이 확정했다.
...
- `Asia/Seoul` 벽시계 5분 경계가 고정 수집 주기다.
- 10분·15분 등 대안 주기는 평가하거나 런타임 옵션으로 제공하지 않는다.
- 중복 관측시각·Raw payload가 발생해도 계획 호출을 건너뛰거나 주기를 바꾸지 않는다.
- 모든 Raw를 보존하고 중복 건수·비율을 기록하며, 제거·선별·가중치는 EG-8
  데이터셋 구성에서 다룬다.
- 주기를 바꾸려면 새 PM 명시 결정, 버전 계약 변경과 별도 코드 검토가 모두 필요하다.

첫 EG-7 1시간 파일럿은 이미 확정된 5분 계약의 구현·운영 안전성을 12개 회차에서
검증한다. 파일럿 날짜·시작시각, 일일 운영시간대, 24시간 또는 선택 시간 운영 여부,
API 할당량·용량, 운영 ID·계획 지문, Live 실행과 첫 1시간 이후 확대 시점은 별도
`OPEN_PM_DECISION`이다. 운영시간 미결정을 주기 미결정으로 해석하지 않는다.

121개 Area 확대가 필요하다고 확인되면 5분 기준을 임의 변경하지 않고 호출량과
운영 가능성을 별도 검토해 PM 승인을 받는다.
...
저장소 최상위 `work log/`는 Issue #47의 PM 승인으로 폐기한다.

- 저장소 안에 해당 디렉터리나 새 작업일지를 생성하지 않는다.
- 새 개인 작업일지는 저장소 밖에서만 관리한다.
- `.gitignore`의 정확한 `/work log/` 규칙 하나만 유지하고, 유사·중첩·일반
  로그 또는 다른 정상 프로젝트 경로까지 제외하는 광범위 ignore 규칙을 금지한다.
- 기존 추적 항목을 허용하는 Legacy 예외는 두지 않는다.
- 보호 경로의 내부 파일명·상대경로·내용·크기·해시를 출력하지 않는다.
- Git stdout·stderr는 메모리에서 캡처하고 원문을 출력하지 않는다. H-203·H-305·
...
- PR #48은 Issue #47 해결과 전체 검증 전까지 Merge하지 않는다.
