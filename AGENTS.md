...
- `encoding="utf-8-sig"`를 사용해 UTF-8과 UTF-8 BOM을 모두 처리한다.
...
- 공식 CSV 배치: 완료
- CSV 무결성 검증: 읽기 전용 검사 완료
- EG-1 통과: 미통과

EG-1 읽기 전용 검사에서 헤더 제외 CSV 레코드 989개와 전 필드 공백
레코드 868개가 확인됐다. 공식 CSV는 변경되지 않았으며, 기준파일 정정 또는
교체 후 재검증을 통과하기 전까지 EG-1 통과로 간주하지 않는다.
...
3. `CATEGORY`, `NO`, `AREA_CD`, `AREA_NM`, `ENG_NM` 컬럼이 모두 존재하는가
