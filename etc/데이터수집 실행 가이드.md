...
> 그대로 유효하다. Python은 이후 정규화·분석·머신러닝에 사용한다. Apps Script의 24시간
> 장기 지속성은 아직 `PENDING_VALIDATION`이며, Apps Script 소스의 Git 버전관리와
> Python 쪽 데이터 통합은 `PLANNED`다. 백업은 수집 실행과 분리된 Google Drive for Desktop
> Sync Backup Worker가 담당하는 목표구조이며 로컬 EG-6B Batch 기준으로는 아직 구현되지
> 않았다.
