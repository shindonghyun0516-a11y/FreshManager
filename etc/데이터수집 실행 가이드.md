...
> 경로는 매 단계 PM이 별도로 승인한다. 백업은 수집 실행과 분리된 Google Drive for Desktop Sync
> Backup Worker가 담당하는 목표구조이며 현재는 아직 구현되지 않았다.
...
운영방식으로 사용하지 않는다. 수집 실행은 로컬 Python에 유지하고, 1회 실행형
...
Google Sheets·Apps Script 수집, 고정 5분 트리거, 승인 없는 수집 Scheduler와 121개
Area 확대는 현행 절차가 아니다. EG-6C도 신설하지 않는다.
