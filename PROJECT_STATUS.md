...
- Area-first Map-centered UI Design Contract: `DRAFT_PENDING_PM_REVIEW`
- Repository Documentation Structure Alignment: `DRAFT_PENDING_PM_REVIEW`
- History Archive and Documentation Index: `DRAFT_PENDING_PM_REVIEW`
...
- 현재 문서 작업: Issue #150 `OPEN · IN_PROGRESS`,
  Branch `docs/issue-150-map-centered-design-doc-alignment`
...
- 파일럿 사용자 선택 Spot 정적 Master: `AVAILABLE_ON_MAIN`(Issue #132, PR #133) —
  ...
  자동추천·추천순위·기본선택은 없다.
- 초기 파일럿 Area 추천 Core:
  `AVAILABLE_ON_MAIN`(Issue #134, PR #135) — 정확한 5개 Area의
...
- Area-first 웹 파일럿 계약:
  `AVAILABLE_ON_MAIN`(D-022, Issue #140, PR #141) — 초기 기본 흐름은 사용자가 승인된
  ...
  파일·배포는 미구현이고 기술 Stack 공식화는 Audit 이후 별도 ADR이다. 이 상태는
  계약 문서가 `main`에 있다는 뜻이며 Area-first Service·API, Vue UI, FastAPI,
  NAVER Map, Spot Prototype Data와 배포가 구현됐다는 뜻이 아니다.
...
- UI/UX Detailed Design: Area-first 계약 `AVAILABLE_ON_MAIN`; 화면 구현 `NOT_STARTED`
...
- Issue #150: `OPEN · IN_PROGRESS`
- Issue #150 Source Branch: `docs/issue-150-map-centered-design-doc-alignment`
- Issue #69: `OPEN`
- Issue #129: `CLOSED`
- Issue #128: `CLOSED`
- Issue #132: `CLOSED`
...
공급은 ADR-012 `ACCEPTED`로 확정했다. Issue #150은 지도 중심 UI 계약과 문서구조
정렬 Draft를 작성 중이며 PM 검토 전에는 완료로 확정하지 않는다. 다음 행동은
다음 순서다.

1. 이번 Design/Docs PR PM 검토·병합
2. Vue·FastAPI 통합 Scaffold
3. Area-first Read-only API·Prototype 공급경로
4. 지도 중심 Responsive UI·API 연결
5. NAVER Map·Geolocation·QA·제한배포
6. 실제 프래시매니저 사용성 검증
...
1. 공식 API 할당량과 rate-limit 호환성을 확인한다.
2. 로컬 Source와 Drive sync-copy preflight를 확인한다.
3. 범위가 고정된 운영 Plan v2 하나를 생성한다.
4. `pilot_run_id` 하나와 Batch ID 정확히 12개를 생성한다.
5. 결정적 Plan fingerprint를 생성·검증한다.
6. 그 정확한 Plan에 대한 한정된 PM Live 승인을 기록한다.
7. 첫 Area-only 1시간 Live 수집을 실행한다.
8. 즉시 기술·데이터 품질 분석을 수행한다.
9. 자동 확대 없이 중단한다.
10. 다음 Area 수집 시간창을 결정한다.
11. 별도 승인된 S-DoT·ML·Spot Workstream을 계속한다.

각 운영 단계는 필요한 확인과 별도 PM 승인을 전제로 하며, 24시간 수집으로 자동
확대하지 않는다.
...
`NOT_STARTED`다. Issue #150의 지도 중심 UI 계약·문서구조·History Index는
`DRAFT_PENDING_PM_REVIEW`이며 구현 완료를 뜻하지 않는다. EG-7 Live
